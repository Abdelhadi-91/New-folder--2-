...
const WorkflowColumn = ({ title, steps }: { title: string; steps: string[] }) => (
  <div className="rounded-xl border border-border bg-card p-5 sm:p-8">
    <div className="flex items-center gap-3 mb-6 sm:mb-8 justify-end">
      <span className="text-primary font-semibold text-sm sm:text-base">{title}</span>
    ...
    <div className="space-y-4 sm:space-y-6">
      {steps.map((step, i) => (
        ...
        >
          <p className="flex-1 text-muted-foreground text-sm sm:text-base">{step}</p>
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-primary/20 border border-primary/50 flex items-center justify-center text-primary font-bold text-xs sm:text-sm shrink-0">
            {i + 1}
          </div>
        </motion.div>
      ))}
